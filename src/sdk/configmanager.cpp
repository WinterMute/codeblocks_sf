...
inline wxString ConfigManager::GetUserDataFolder()
{
    ...
    if (!ConfigManager::has_alternate_user_data_path && ::GetEnvironmentVariable(_T("APPDATA"), buffer, MAX_PATH))
        return wxString::Format(_T("%s\\devkitIDE"), buffer);
    else
        return wxStandardPathsBase::Get().GetUserDataDir();
#else
#ifdef __linux__
    gchar *filename = g_build_filename(g_get_user_config_dir(), "devkitide", nullptr);
...
void ConfigManager::InitPaths()
{
    ...
    {
        if (platform::windows)
            ConfigManager::data_path_global = app_path + _T("\\share\\devkitide");
        else if (platform::macosx)
            ConfigManager::data_path_global = res_path + _T("/share/devkitide");
        else
    ...
    if (plugin_path_global.IsEmpty())
    {
        ...
        else
        {
            ...
            ConfigManager::plugin_path_global = ((const wxStandardPaths&)wxStandardPaths::Get()).GetInstallPrefix() + _T("/lib/devkitide/plugins");
            ...
            if (!wxDirExists(ConfigManager::plugin_path_global) && wxIsPlatform64Bit())
            {
                ...
                ConfigManager::plugin_path_global = ((const wxStandardPaths&)wxStandardPaths::Get()).GetInstallPrefix() + _T("/lib64/devkitide/plugins");
            }
#endif // __WXGTK__
        }
    }
    ...
    ConfigManager::data_path_user = dataPathUser + wxFILE_SEP_PATH + _T("devkitide");
...
void ConfigManager::MigrateFolders()
{
    ...
    wxString newConfigFolder = wxString::FromUTF8(g_build_filename (g_get_user_config_dir(), "devkitide", NULL));
    ...
    wxString oldDataFolder = oldConfigFolder + wxFILE_SEP_PATH + _T("share") + wxFILE_SEP_PATH + _T("devkitide");
    wxString newDataFolder = wxString::FromUTF8(g_build_filename (g_get_user_data_dir(), NULL)) + wxFILE_SEP_PATH + _T("devkitide");
    wxString msg;
