...
CompilerdevkitA64::CompilerdevkitA64()
    : Compiler(_("devkitA64"), _T("devkita64-gcc"))
{
    m_Weight = 8;
    Reset();
}
...
Compiler * CompilerdevkitA64::CreateCopy()
{
    return (new CompilerdevkitA64(*this));
}

AutoDetectResult CompilerdevkitA64::AutoDetectInstallationDir()
{
    wxString sep = wxFileName::GetPathSeparator();
    // look first if devkitARM was installed with Code::Blocks
    wxString testPath = ConfigManager::GetExecutableFolder();
    if (!wxFileExists(testPath + sep + _T("devkitA64") + sep + _T("bin") + sep + m_Programs.C))
    {
        testPath.Truncate(0);
    ...
    m_MasterPath = testPath + sep + _T("devkitA64");
    AutoDetectResult ret = wxFileExists(m_MasterPath + sep + _T("bin") + sep + m_Programs.C) ? adrDetected : adrGuessed;
    if (ret == adrDetected)
    {
        AddIncludeDir(testPath + sep + _T("devkitA64") + sep + _T("aarch64-none-elf") + sep + _T("include"));
        if (platform::windows)
